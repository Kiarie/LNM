...
namespace iPayLNM_MySQL
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            string strSQL = "SELECT * FROM `rexol`.`sms_in` WHERE `sms_text` LIKE '%Confirmed.%' AND `sender_number` IS NOT NULL AND (`code` = '' OR `code` IS NULL)";
            ...
            while (dr.Read())
            {
                ...
                */
                ...
                strSQL = "Update `rexol`.`sms_in` SET `code` = '" + arrText[0] + "', `amount` = '" + arrText[5] + "', `firstname` = '" + arrText[9] + "', `lastname` = '" + arrText[10] + "', `msisdn` = '" + arrText[8] + "' WHERE `id` = " + ID1;
